#lang typed/racket
(require "factors.rkt" "random-var.rkt" "utils.rkt" threading math/flonum "model.rkt")
...
(define (create-factor-from-data [var : DiscreteRandomVar]
...
(define (mle-train-model! [model : Model]
  ...
  (for ([v (filter DiscreteRandomVar? (hash-values (Model-vars model)))])
